  ...
  <div class="weather-assignment">
    ...
    <section class="dashboard-card">
      ...
      <div v-if="searchHistory.length" class="search-history">
        ...
        >
          {{ item }}
          <span class="history-remove" @click.stop="removeHistoryItem(item)">✕</span>
        </button>
      </div>
    </section>
    ...
    <section class="dashboard-card">
      ...
      <p v-if="hottestCity && coolestCity" class="temp-summary">
        <template v-if="hottestCity.id === coolestCity.id">
          🌡️ {{ hottestCity.name }} {{ hottestCity.temp }}°C
        </template>
        <template v-else>
          🔥 가장 더운 곳: {{ hottestCity.name }} {{ hottestCity.temp }}°C · ❄️ 가장 시원한 곳:
          {{ coolestCity.name }} {{ coolestCity.temp }}°C
        </template>
      </p>
      ...
      <!-- 5. 검색 결과 없음 안내 (v-for와 형제 태그로 분리) -->
...
// 정렬 + 즐겨찾기 필터를 합쳐서 실제 화면에 뿌려줄 목록 (filteredWeatherList 기반)
...
// 7. 검색 기록 (800ms 디바운스, 최대 5개, 중복/빈값 제외)
